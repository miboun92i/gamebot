...
async def top_cmd(m:Message):
    ...
    sid=await current_season_id()
    rows=await pool.fetch("""SELECT u.user_id,u.name,s.xp FROM player_season_stats s
      JOIN users u ON u.chat_id=s.chat_id AND u.user_id=s.user_id
    ...
    season=await pool.fetchrow("SELECT label,starts_at,ends_at FROM seasons WHERE id=$1",sid)
    total=await pool.fetchval("SELECT count(*) FROM player_season_stats WHERE chat_id=$1 AND season_id=$2",m.chat.id,sid)
    players=[]
    for r in rows[:7]:
        avatar_bytes=None
        try:
            photos=await m.bot.get_user_profile_photos(r["user_id"],limit=1)
            ...
            pass
        players.append({"name":r["name"],"xp":r["xp"],"rank":rank_for_xp(r["xp"]),"avatar":avatar_bytes})
    group_avatar=None
    try:
        chat=await m.bot.get_chat(m.chat.id)
        if chat.photo:
            buf=BytesIO()
            await m.bot.download(chat.photo.big_file_id,destination=buf)
            group_avatar=buf.getvalue()
    except Exception:
        pass
    ...
    start_at=season["starts_at"].astimezone(TZ)
    month_label=start_at.strftime("%B %Y")
    fr_months={"January":"Janvier","February":"Février","March":"Mars","April":"Avril","May":"Mai","June":"Juin","July":"Juillet","August":"Août","September":"Septembre","October":"Octobre","November":"Novembre","December":"Décembre"}
    for en,fr in fr_months.items(): month_label=month_label.replace(en,fr)
    label=str(season["label"])
    season_number=label.split()[-1] if label.lower().startswith("saison ") else label
    image=render_top(m.chat.title,season_number,month_label,players,total,top10_xp,remaining_seconds,group_avatar)
